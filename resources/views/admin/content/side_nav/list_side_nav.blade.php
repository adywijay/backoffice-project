@section('sidenav-content')
            ...
            <ul class="menu-list flex-grow-1 mt-3">
                <li class="collapsed">
                    ...
                    <ul class="sub-menu collapse show" id="dashboard-page">
                        <li><a class="ms-link active" href="javascript:void(0)" id="home_admin"> <span>Home</span></a></li>
                ...
                <li class="collapsed">
                    ...
                    <ul class="sub-menu collapse" id="employee-monitoring">
                        <li><a class="ms-link" href="javascript:void(0)" id="mapping_task"><span>Tasks</span></a>
                        </li>
                        <li><a class="ms-link" href="javascript:void(0)" id="list_task"><span>Task List</span></a></li>
                    </ul>
                </li>
                <li class="collapsed">
                    <a class="m-link" data-bs-toggle="collapse" data-bs-target="#employee-evaluation"
                        href="javascript:void(0)"><i class="icofont-users-alt-5"></i> <span>Employees</span> <span
                            class="arrow icofont-dotted-down ms-auto text-end fs-5"></span></a>
                    <ul class="sub-menu collapse" id="employee-evaluation">
    ...
    <script type='text/javascript'>
        ...
        $('#home_admin').click(function() {
            window.location.replace('{{ route('home_ad') }}');
        });

        $('#event_officer').click(function() {
            window.location.replace('{{ route('event_officer_ad') }}');
        });

        $('#event_management').click(function() {
            window.location.replace('{{ route('get_event_management_ad') }}');
        });

        $('#leave_management').click(function() {
            window.location.replace('{{ route('leave_management_ad') }}');
        });

        $('#employe-indicators').click(function() {
            window.location.replace('{{ route('employe_perform_ad') }}');
        });

        $('#mapping_task').click(function() {
            window.location.replace('{{ route('mapping_task_ad') }}');
        });

        $('#list_task').click(function() {
            window.location.replace('{{ route('list_task_ad') }}');
        });
    </script>
@endsection
